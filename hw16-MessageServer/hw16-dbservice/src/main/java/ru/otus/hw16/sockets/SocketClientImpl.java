...
public class SocketClientImpl implements SocketClient {
  ...
  private void run() {
    try {
      while (clientSocket.isConnected()) {
        ...
        MessageTransport messageIn = new Gson().fromJson(resp, MessageTransport.class);
        logger.info("message from ms: {}", messageIn);
        fromMS.put(messageIn);

        MessageTransport messageOut = forMS.take();
  ...
  @Override
  public MessageTransport getMessageFromMS() {
    try {
      return fromMS.take();
    } catch (InterruptedException e) {
      logger.error(e.getMessage(),e);
